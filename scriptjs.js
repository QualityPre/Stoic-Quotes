...
async function getStoicQuote(url) {
  try {
    ...
    if (quote.length > 150) quoteContainerEl.classList.add("small");
...
async function getStoicQuoteTwo(url) {
  try {
    ...
    if (quote.length > 150 && quote.length < 300)
      quoteContainerEl.classList.add("small");
    if (quote.length > 300) quoteContainerEl.classList.add("very--small");
    createQuote(quote, author);
  } catch (err) {
    console.log(err);
  }
